...
\title{Puzzle RL Theory:\\Athena-Backed RL vs.\ SFT Mechanism Map}
\author{Murphy}
\date{2026-04-12 02:57 UTC}
...
\item \textbf{Athena-backed decision rule:} do not call a later result ``support expansion'' unless the post-RL policy emits trace-valid successful trajectories on a fixed hard slice that the pre-RL model still cannot recover under larger-k search and search-plus-distill controls.
...
\subsection*{Athena-backed support-expansion rule}

A narrow Athena follow-up on the same fixed evidence set sharpened the part that was still too vague in the first draft. The operational criterion is:

\begin{quote}
Only call a result ``support expansion'' if, on a fixed hard slice, the post-RL policy emits trace-valid successful trajectories at a non-trivial rate that the pre-RL model still cannot recover under larger-k search and positive-support distillation controls.
\end{quote}

The strongest evidence, ordered from stronger to weaker, is:

\begin{enumerate}
\item the base model still has near-zero success on the hard slice even after aggressive search, while the post-RL policy now succeeds directly;
\item the gain survives best-of-N plus cloning and reward-free online self-training controls;
\item the new successful traces live in clusters that were absent or vanishingly rare in base samples and early RL checkpoints;
\item step-level trace validity improves, not only final-answer correctness;
\item pass@1 rises without pass@k or diversity collapsing.
\end{enumerate}

What does \emph{not} count on its own:

\begin{enumerate}
\item higher final-answer accuracy alone;
\item higher pass@1 when large-k base-model search already nearly matches it;
\item the fact that RL found good trajectories at all, if search plus cloning can already recover them;
\item gains reproduced by reward-free online self-training;
\item longer or more natural-looking traces.
\end{enumerate}
...
\item \textbf{Search-control hypothesis:} if an apparent RL gain disappears once we add matched large-k search and best-of-N plus cloning controls, the original gain was mostly search amortization rather than new support.
\item \textbf{Trace-validity hypothesis:} if final-answer gains appear without better trace validity on the hard slice, the method improved answer selection or concentration rather than the process object we care about.
...
\begin{quote}
Current LLM RL often behaves like on-policy distribution shaping over capabilities that are already latent in the base model. The main exceptions appear when better data refresh, better verifier-filtered search, or better credit assignment materially change the learning problem. The maze project should be built to separate those ingredients one by one, and it should only use ``support expansion'' language when matched search and distillation controls clearly fail.
\end{quote}
